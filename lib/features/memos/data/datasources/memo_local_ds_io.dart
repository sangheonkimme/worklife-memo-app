...
class MemoLocalDataSource {
  MemoLocalDataSource({Future<Isar>? isar})
    : _isarFuture =
          isar ??
  ...
  Future<void> togglePin({required int id, required bool isPinned}) async {
    ...
    await isar.writeTxn(() async {
      ...
      if (entity != null) {
        ...
        entity.searchText = MemoEntity.buildSearchText(
          entity.title,
          entity.content,
        );
  ...
  Stream<List<MemoEntity>> _watch({
    ...
    return Stream.fromFuture(_isarFuture).asyncExpand((isar) {
      ...
      if (pinnedOnly) {
        filters.add(
          const FilterCondition.equalTo(property: 'isPinned', value: true),
        );
      }

      if (folderId != null) {
        filters.add(
          FilterCondition.equalTo(property: 'folderId', value: folderId),
        );
      }
